...
class RBM_MSE(object):
	...
	def Binarization(self,h):
		k=100
		return T.nnet.sigmoid(k*(h-0.5))
	...
	def get_cost_updates(self, k=1, MSEWeight=1):
		...
		gparams_Rec = T.grad(self.MSECost, self.params)	
		#pdb.set_trace()
		grad = gparams[:]
		for id in range(len(gparams)):
			grad[id] = gparams[id] + MSEWeight*gparams_Rec[id]
			
			
		# set the computational graph, real training config parameters will be imported in the training function
		updates[self.params_inc[0]] = self.params_inc[0]*self.momentum - (grad[0]  + self.W*self.weightCost)* self.lr
		updates[self.params_inc[1]] = self.params_inc[1]*self.momentum - grad[1] * self.lr   
		updates[self.params_inc[2]] = self.params_inc[2]*self.momentum - grad[2] * self.lr
		...
		if self.MSEType=='mode':
			MSE_cost = self.get_reconstruction_cost(self.mode_recover)
		else:
			MSE_cost = self.get_reconstruction_cost(self.recover)

		return monitoring_cost, MSE_cost, updates		
		# end-snippet-4		
	...
	def PrintTrainingParameters(self,dataDir,modelFile,MSEWeight=1,learnRate=0.001,batch_size=128,initialMomentum=0.5, finalMomentum=0.9, weightCost=0.0002, layerEpochs=100,CDstep=1):
		print '    Training parameters:'
		print '        MSEType: ',self.MSEType
		print '        dataDir: ',dataDir
		print '        modelFile: ',modelFile
		print '        MSEWeight: ',MSEWeight
	...
	def train(self,dataDir,modelFile,MSEWeight=1,learnRate=0.001,batch_size=128,initialMomentum=0.5, finalMomentum=0.9, weightCost=0.0002, layerEpochs=100):
		CDstep=1
		#pdb.set_trace()
		self.PrintTrainingParameters(dataDir,modelFile,MSEWeight=MSEWeight,learnRate=learnRate,batch_size=batch_size,initialMomentum=initialMomentum, finalMomentum=finalMomentum, weightCost=weightCost, layerEpochs=layerEpochs,CDstep=CDstep)
		
		cost,MSE_cost,updates = self.get_cost_updates(CDstep,MSEWeight)  
		...
		for epoch in range(layerEpochs):			  
			...
			for item in fileList:  
				...
				for index in range(n_train_batches):					
					tmp = train(data[index * batch_size: (index + 1) * batch_size])	
					mean_cost += [tmp[0]]
					MSE_cost += [tmp[1]]		
				#print mean_cost[-1]
			...
			print ' average cost per sample is %f, %s reconstruction cost is %f, epoch time: %f seconds'%(numpy.mean(mean_cost), self.MSEType, np.mean(MSE_cost),epoch_end - epoch_start)
